...
public class TeamService {
			...
			teams = teams.stream().map(team -> {
				...
				team.setMatches(matchRepository
						.getByTeam1AndTeam2OrTeam2AndTeam1OrderByDateDesc(
								teamName1, teamName2, teamName1, teamName2, cp,
								pageSize));
				return team;
			}).collect(Collectors.toList());
	...
	public List<Team> getTeamByName1(String teamName1, int cp, String venue,
		...
		if (!venue.isEmpty()) {
			...
			teams = teams.stream().map(team -> {
				team.setMatches(matchRepository
						.getByVenueAndTeam1OrVenueAndTeam2OrderByDateDesc(venue,
								teamName1, venue, teamName1, cp, pageSize));
				teamsAll.stream()
			...
			Team team = matchRepository.countTotalMatchesByTeam1(teamName1, cp,
					pageSize);
			List<Team> teams = new ArrayList<>();
			teams.add(team);
			team.setTotalWinsPercent(populateWinsPercent(team.getTotalMatches(),
					team.getTotalWins()));
			return teams;
			// List<ITeamCount> objs2 =
			// matchRepository.countTotalMatchesByTeam2(teamName1);
			// List<Team> teams = getTeams(objs1, objs2);
			// teams = teams.stream().map(team -> {
			// team.setMatches(matchRepository.getByTeam1OrTeam2OrderByDateDesc(teamName1,
			// teamName1, cp, pageSize)
			// .getContent());
			...
			// teamsAll.stream().filter(x ->
			// !x.getName().equalsIgnoreCase(teamName1)).forEach(x -> {
			// List<Team> temp = getTeamByName1AndName2(teamName1, x.getName(),
			// 0, venue, 3);
			// if (temp != null && !temp.isEmpty()) {
			// temp.get(0).setName(x.getName());
			// team.getTeams().addAll(temp);
