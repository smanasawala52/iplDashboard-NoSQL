...
import com.example.ipldashboardnosql.model.TeamBuilder;
import com.example.ipldashboardnosql.repository.MatchRepository;
import com.example.ipldashboardnosql.repository.TeamRepository;
...
public class TeamService {
	...
	public List<Team> getTeamByName1AndName2(String teamName1, String teamName2, int cp, String venue, int pageSize) {
		...
		if (!venue.isEmpty()) {
			...
			teams = teams.stream().map(team -> {
				team.setMatches(matchRepository.getByVenueAndTeam1AndTeam2OrVenueAndTeam2AndTeam1OrderByDateDesc(venue,
						teamName1, teamName2, venue, teamName1, teamName2, cp, pageSize).getContent());
				return team;
			}).collect(Collectors.toList());
			...
			teams = teams.stream().map(team -> {
				...
				team.setMatches(matchRepository.getByTeam1AndTeam2OrTeam2AndTeam1OrderByDateDesc(teamName1, teamName2,
						teamName1, teamName2, cp, pageSize).getContent());
				return team;
			}).collect(Collectors.toList());
	...
	public List<Team> getTeamByName1(String teamName1, int cp, String venue, int pageSize) {
		...
		if (!venue.isEmpty()) {
			...
			List<Team> teams = getTeams(objs1, objs2);
			teams = teams.stream().map(team -> {
				team.setMatches(matchRepository.getByVenueAndTeam1OrVenueAndTeam2OrderByDateDesc(venue, teamName1,
						venue, teamName1, cp, pageSize).getContent());
			...
			List<Team> teams = getTeams(objs1, objs2);
			teams = teams.stream().map(team -> {
				team.setMatches(matchRepository.getByTeam1OrTeam2OrderByDateDesc(teamName1, teamName1, cp, pageSize)
						.getContent());
